...
public class DataStructure implements IDataStructure, IDataType, Cloneable {
	...
	private void createItem(TypeLineParser parser) throws DataItemCreationException, DataImportException {
		...
		if (itemType != null) {
			if (itemType.equalsIgnoreCase("fillup")) {
				try {
					this.fillUp(Integer.parseInt(itemData));
				} catch (NumberFormatException e) {
					throw new DataImportException("could not parse int: " + itemData);
				}
			} else {
				IDataType item = DataItemFactory.getInstance().createDataItem(itemType);
				if (item != null) {
					this.addItem(item);
					item.setName(parser.getItemName());
					item.importDataString(itemData);
				}
			}
		}
	}
	...
	@Override
	public void importStructure(IDataStructure struct) throws DataImportException {
		// Elemente nach deren Namen zuordnen und Daten übernehmen. Unbenannte
		// Elemente werden nicht berücksichtigt.
		if (struct != null) {
			for (Iterator<IDataType> it = struct.getItems().iterator(); it.hasNext();) {
				IDataType fitem = it.next();
				String fname = fitem.getName();
				if (fname != null && !fname.isEmpty()) {
					IDataType mitem = this.getItemByName(fname);
					if (mitem != null) {
						mitem.importDataString(fitem.exportDataString());
					}
				}
			}
		}
	...
	protected IDataType getItemByName(String name) {
		if (name != null) {
			for (Iterator<IDataType> it = this.getItems().iterator(); it.hasNext();) {
				IDataType item = it.next();
